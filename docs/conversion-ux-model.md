# Conversion UX Model - fde-engagement-playbook
...
| Primary buyer or user | AI consultancies, technical founders, field engineering teams, and implementation partners. |
| Value wedge | Complex AI deployments often fail in discovery, acceptance criteria, stakeholder alignment, or handoff rather than model quality. |
...
| First action | Open the README archived-status note and enterprise-readiness note, then inspect Templates are complete and easy to copy only if this older domain proof is relevant. |
...
- Optional entry: Engagement design workshop ($5k-$18k) when this archived proof exactly matches the buyer problem.
- Revival step: FDE operating kit ($15k-$60k) only after the active successor and support boundary are clear.
- Longer-term path: Partner playbook license ($2k-$12k/month) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Workshop close rate only as a discussion metric until fresh validation exists.
